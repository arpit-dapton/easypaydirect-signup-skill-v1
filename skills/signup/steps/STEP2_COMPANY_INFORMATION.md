...
>
> ⚠️ **Evaluate this show/require/clear logic at the moment Step 2 becomes visible (right after Step 1's submission succeeds), not once at page load.** On a fresh signup, Step 1's country isn't known yet when the page/script first loads — it only becomes known the instant Step 1 succeeds. If `business_register_number`'s visibility is computed a single time on page load (e.g. inside a `$(document).ready(...)`/`initStep2()` that runs before Step 1 has ever been submitted), it locks in based on whatever country value happens to be persisted at that moment — none for a truly fresh session, or a stale value left over from an earlier abandoned/different test signup in the same browser — and never gets re-checked once the real country is known. The field can then stay hidden and empty for a merchant who actually picked a non-US country, while the submission code (which correctly re-reads the persisted country fresh at submit time) still expects to send a value — producing a 422 `business_register_number is required for non-US companies` even though the merchant never had a chance to fill it in. Re-run the show/require/clear check for `business_register_number` (and the `federal_tax_id` label/requiredness check, which has the same dependency) every time Step 2 is entered/shown, using the country value as it stands right then — not a value captured earlier.
