...
> 3. **Quick start, then we email you a link** — the merchant only enters their basic contact info on your website, just like option 2. But instead of being taken straight to EasyPayDirect, they get an email with a link to continue their application on EasyPayDirect whenever they're ready.
...
| **2. Quick start, then handed off** | Supported, no new backend endpoint needed. Internally this means: only scaffold Step 1's page/form — **do not build Steps 2–6 at all.** The merchant leaves this site the moment Step 1 succeeds, so any Step 2–6 UI would be dead code, never reached. **Also strip every "N of 6" / multi-step signal from the one page you do build** — no 6-dot progress bar, no "Step 1 of 6" subtitle, no step counter of any kind. There is exactly one step on this site; showing "1 of 6" falsely promises the merchant 5 more steps here before they get redirected away entirely. If you want a progress affordance at all, it must not imply steps beyond this one (e.g. a plain "Create your account" heading with no counter). After a successful Step 1 submission, redirect the browser straight to `{EMAP_APP_URL}/upload-document/{uuid}?redirect=1` (using the `uuid` returned by `POST /api/v1/signup`) — this is EasyPayDirect's own existing auto-login + resume route (the same one its admin dashboard's "Resume Merchant Signup" button already links to). See "Flow Option 2 & 3 Backend Endpoints" below. |
| **3. Quick start, then we email you a link** | Supported. Internally this is the same Step-1-only build as option 2 — **only scaffold Step 1's page/form; do not build Steps 2–6 at all**, and **strip every "N of 6" / multi-step signal** (no 6-dot progress bar, no "Step 1 of 6" subtitle, no step counter of any kind) exactly as described for option 2. The only difference from option 2 is the handoff. Instead of redirecting the browser, on a successful Step 1 submission (`POST /api/v1/signup`, payload includes `"step_count": 1` and, if provided, `partner_key`) immediately call `POST /api/v1/signup/resume-link` with the merchant's `email` — read it silently from the just-submitted Step 1 data, **never prompt the merchant to type/re-enter it**. Then show a "check your email" confirmation view telling them to click the link in their inbox to finish their application on EasyPayDirect. Persist a completed flag (e.g. `localStorage.setItem('signup_completed', 'true')`) *before* rendering that view so a page refresh re-shows the confirmation, not a blank Step 1. See "Flow Option 2 & 3 Backend Endpoints" below. |
...
- **Option 3 (email a resume link after Step 1)** uses `POST /api/v1/signup/resume-link` — body `{"email": "<merchant's email>"}`. Returns `{"status":true,"message":"Resume link sent"}` (200, always — mirrors the existing EasyPayDirect "finish later" feature's own behavior of never revealing whether an email matches an account) and emails the merchant a resume link via EasyPayDirect's existing "finish later" template; `422` for a missing/invalid email, `429` if rate-limited (5 requests / 5 minutes per IP). This is EasyPayDirect's existing `SignupController::sendFinishLaterLink` logic reused as-is, just with its `auth()` session check swapped for an email lookup since a partner-hosted form's visitor has no EasyPayDirect session. Call it once, automatically, immediately after the Step 1 `POST /api/v1/signup` succeeds — the application must already exist for the email lookup to resolve, so never call it before Step 1 is submitted.
